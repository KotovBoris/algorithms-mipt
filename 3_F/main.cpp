...
long long BinSearch(size_t number, const std::vector<size_t>& sorted) {
  long long left = -1;
...
public:
  ...
  SegmentTree(size_t length)
    : data(std::vector<size_t>(length)), leaves_(length) {
    nodes_.reserve(4 * length);
  }
...
private:
  ...
  long long MergeFundamental(size_t min, size_t max, size_t start, size_t end,
    ...
    if (start == node.start && end == node.end) {
      return BinSearch(max, node.value) - BinSearch(min - 1, node.value);
    }
...
void SegmentTree::BuildTree() {
  nodes_.push_back(Node{ 0, data.size() - 1, {}, nullptr, nullptr, nullptr });
...
int main() {
  ...
  for (size_t i = 0; i < request_number; i++) {
    ...
    std::cout << tree.GetValue(--start, --end, min, max) << '\n';
  }
}
